...
public:
  ...
  void operator()(sycl::item<1> item) const
  {
    ...
    ValueType Hh, Hu, Hv;
    int b = 0;
    if (y_m > z_f and y_p > z_f) {
      // Fully submerged case
      b = 1;
      ValueType Fh_m = h_m * (u_m * xdir + v_m * ydir);
      ValueType Fh_p = h_p * (u_p * xdir + v_p * ydir);
      ValueType Fu_m = u_m * (0.5f * u_m * xdir + v_m * ydir) + 9.81f * h_m * xdir;
      ValueType Fu_p = u_p * (0.5f * u_p * xdir + v_p * ydir) + 9.81f * h_p * xdir;
      ValueType Fv_m = v_m * (0.5f * v_m * ydir + u_m * xdir) + 9.81f * h_m * ydir;
      ValueType Fv_p = v_p * (0.5f * v_p * ydir + u_p * xdir) + 9.81f * h_p * ydir;
      ...
      if (z_m > z_p) {
	b = 2;
	ValueType Fh = h_m * (u_m * xdir + v_m * ydir);
	ValueType Fu = u_m * (0.5f * u_m * xdir + v_m * ydir) + 9.81f * h_m * xdir;
	ValueType Fv = v_m * (0.5f * v_m * ydir + u_m * xdir) + 9.81f * h_m * ydir;
	...
	b = 3;
	ValueType Fh = h_p * (u_p * xdir + v_p * ydir);
	ValueType Fu = u_p * (0.5f * u_p * xdir + v_p * ydir) + 9.81f * h_p * xdir;
	ValueType Fv = v_p * (0.5f * v_p * ydir + u_p * xdir) + 9.81f * h_p * ydir;
